...
// On Vercel, use /tmp/ for writable upload storage
...
router.patch('/:id', auth, upload.single('file'), (req, res) => {
  ...
  if (req.file) { updates.file_url = `/api/assets/uploads/${req.file.filename}`; updates.file_name = req.file.originalname; }
